...
const ExpenseManagement = ({ showProjectSections = true, projectsOnly = false }) => {
    ...
    const [projectForm, setProjectForm] = useState(() => ({ name: '', clientName: '', clientPhone: '', clientTotal: '', clientReceived: '', developers: [emptyDeveloper()], companies: [emptyCompany()], status: 'processing', startDate: freshToday(), completionDate: '', description: '', assignedTeachers: [] }));
    ...
    const loadData = async () => {
        try {
            ...
            const [financeRes, projectsRes, teachersRes] = await Promise.all([financeAPI.getAll(params), financeAPI.getProjects(), userAPI.getVerifiedByRole('teacher')]);
    ...
    const openNewProject = () => { setEditingProjectId(null); setProjectForm({ name: '', clientName: '', clientPhone: '', clientTotal: '', clientReceived: '', developers: [emptyDeveloper()], companies: [emptyCompany()], status: 'processing', startDate: freshToday(), completionDate: '', description: '', assignedTeachers: [] }); setShowProjectForm(true); };
    const openEditProject = project => {
        ...
        setProjectForm({
            ...
            assignedTeachers: (project.assignedTeachers || []).map(t => t._id || t)
        });
    ...
    const toggleTeacher = teacherId => setProjectForm(previous => {
        const assigned = previous.assignedTeachers || [];
        const updated = assigned.includes(teacherId) ? assigned.filter(id => id !== teacherId) : [...assigned, teacherId];
        return { ...previous, assignedTeachers: updated };
    });
    const saveProject = async event => {
        ...
        try {
            const payload = {
                ...
                assignedTeachers: projectForm.assignedTeachers || []
            };
    ...
    return (
        <div className="p-4 md:p-6 space-y-6">
            ...
            {showProjectForm && <div className="fixed inset-0 z-[110] bg-black/60 flex items-center justify-center p-4"><form onSubmit={saveProject} className="w-full max-w-4xl max-h-[92vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-2xl p-5 space-y-4 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-lg font-black dark:text-white">{editingProjectId ? 'Edit Project' : 'Create Project'}</h2><p className="text-xs text-gray-400">Track project value, team cost and your expected profit.</p></div><button type="button" onClick={() => setShowProjectForm(false)}><X className="w-5 h-5 dark:text-white" /></button></div><div className="grid md:grid-cols-2 gap-3"><input placeholder="Project name" value={projectForm.name} onChange={event => setProjectForm({ ...projectForm, name: event.target.value })} className={fieldClass} /><input placeholder="Client name" value={projectForm.clientName} onChange={event => setProjectForm({ ...projectForm, clientName: event.target.value })} className={fieldClass} /><input inputMode="tel" placeholder="Client phone number" value={projectForm.clientPhone} onChange={event => setProjectForm({ ...projectForm, clientPhone: event.target.value })} className={fieldClass} /><input inputMode="numeric" placeholder="Client total amount" value={projectForm.clientTotal} onChange={event => updateProjectTotal(event.target.value)} className={fieldClass} /><input type="date" value={projectForm.startDate} onChange={event => setProjectForm({ ...projectForm, startDate: event.target.value })} className={fieldClass} /></div><div className="rounded-2xl border border-gray-100 dark:border-slate-700 p-4"><div className="flex items-center justify-between mb-3"><div><h3 className="font-black dark:text-white">Team Members</h3><p className="text-[10px] text-gray-400">Enter percentage or Rs amount; the other value calculates automatically.</p></div><button type="button" onClick={() => setProjectForm(previous => ({ ...previous, developers: [...previous.developers, emptyDeveloper()] }))} className="px-3 py-2 rounded-xl bg-primary/10 text-primary text-xs font-black flex items-center gap-2"><UserRoundPlus className="w-4 h-4" /> Add Team Member</button></div><div className="space-y-2">{projectForm.developers.map((developer, index) => <div key={index} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_.7fr_1fr_auto] gap-2"><input placeholder="Name" value={developer.name} onChange={event => updateDeveloper(index, 'name', event.target.value)} className={fieldClass} /><input placeholder="Designation" value={developer.designation} onChange={event => updateDeveloper(index, 'designation', event.target.value)} className={fieldClass} /><input inputMode="decimal" placeholder="Percentage %" value={developer.percentage} onChange={event => updateDeveloperShare(index, 'percentage', event.target.value)} className={fieldClass} /><input inputMode="numeric" placeholder="Total payable (Rs)" value={developer.totalPayable} onChange={event => updateDeveloperShare(index, 'totalPayable', event.target.value)} className={fieldClass} /><button type="button" disabled={projectForm.developers.length === 1} onClick={() => setProjectForm(previous => ({ ...previous, developers: previous.developers.filter((_, developerIndex) => developerIndex !== index) }))} className="p-3 text-rose-500 disabled:opacity-30"><Trash2 className="w-4 h-4" /></button></div>)}</div></div><div className="rounded-2xl border border-gray-100 dark:border-slate-700 p-4"><div className="flex items-center justify-between mb-3"><div><h3 className="font-black dark:text-white">Companies</h3><p className="text-[10px] text-gray-400">Add companies with the same percentage or Rs calculation.</p></div><button type="button" onClick={() => setProjectForm(previous => ({ ...previous, companies: [...previous.companies, emptyCompany()] }))} className="px-3 py-2 rounded-xl bg-primary/10 text-primary text-xs font-black flex items-center gap-2"><UserRoundPlus className="w-4 h-4" /> Add Company</button></div><div className="space-y-2">{projectForm.companies.map((company, index) => <div key={index} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_.7fr_1fr_auto] gap-2"><input placeholder="Company name" value={company.name} onChange={event => updateCompany(index, 'name', event.target.value)} className={fieldClass} /><input placeholder="Designation / Service" value={company.designation} onChange={event => updateCompany(index, 'designation', event.target.value)} className={fieldClass} /><input inputMode="decimal" placeholder="Percentage %" value={company.percentage} onChange={event => updateCompanyShare(index, 'percentage', event.target.value)} className={fieldClass} /><input inputMode="numeric" placeholder="Total payable (Rs)" value={company.totalPayable} onChange={event => updateCompanyShare(index, 'totalPayable', event.target.value)} className={fieldClass} /><button type="button" disabled={projectForm.companies.length === 1} onClick={() => setProjectForm(previous => ({ ...previous, companies: previous.companies.filter((_, companyIndex) => companyIndex !== index) }))} className="p-3 text-rose-500 disabled:opacity-30"><Trash2 className="w-4 h-4" /></button></div>)}</div></div><div className={`rounded-2xl border p-4 flex items-center justify-between ${projectRemainingValue < 0 ? 'bg-rose-50 border-rose-200 dark:bg-rose-950/20' : 'bg-emerald-50 border-emerald-200 dark:bg-emerald-950/20'}`}><div><p className="text-[10px] font-black uppercase tracking-wider text-gray-500">Remaining Amount</p><p className="text-xs text-gray-400 mt-1">Client Total − Team Payable − Company Payable</p></div><p className={`text-xl font-black ${projectRemainingValue < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>{money(projectRemainingValue)}</p></div><textarea rows="3" placeholder="Project notes / scope" value={projectForm.description} onChange={event => setProjectForm({ ...projectForm, description: event.target.value })} className={fieldClass} /><div className="rounded-2xl border border-gray-100 dark:border-slate-700 p-4"><div className="flex items-center justify-between mb-3"><div><h3 className="font-black dark:text-white">Assign Teachers</h3><p className="text-[10px] text-gray-400">Select teachers who can view this project (read-only access).</p></div>{(projectForm.assignedTeachers || []).length > 0 && <span className="px-2.5 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-black">{projectForm.assignedTeachers.length} selected</span>}</div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">{activeTeachers.map(teacher => { const isSelected = (projectForm.assignedTeachers || []).includes(teacher._id); return <button key={teacher._id} type="button" onClick={() => toggleTeacher(teacher._id)} className={`flex items-center gap-2.5 rounded-xl border p-2.5 text-left transition-all ${isSelected ? 'border-primary bg-primary/5 ring-1 ring-primary/25' : 'border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-800 hover:border-gray-300'}`}><ProfileAvatar src={teacher.photo} name={teacher.name} size="sm" shape="rounded-lg" /><div className="min-w-0"><p className="text-xs font-bold text-gray-900 dark:text-white truncate">{teacher.name}</p><p className="text-[10px] text-gray-400 truncate">{teacher.email}</p></div>{isSelected && <CheckCircle2 className="ml-auto h-4 w-4 shrink-0 text-primary" />}</button>})}</div>{!activeTeachers.length && <p className="text-xs text-gray-400 text-center py-2">No active teachers found.</p>}</div><button disabled={saving} className="w-full py-3 bg-primary text-white rounded-xl font-black disabled:opacity-50">{saving ? 'Saving...' : editingProjectId ? 'Update Project' : 'Create Project'}</button></form></div>}
        </div>
    );
};
